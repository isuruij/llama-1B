...
- Built a B200-optimized continual pre-training pipeline for this purpose.
...
- The 1B and 3B pipelines (CPT + QA LoRA adapters + extended tokenizer) produced
  merged Sinhala QA checkpoints at both scales
  (`isji/sinllama-1b-qa-v6-merged` and `isji/sinllama-3b-qa-v6-merged`). Full
  per-question transcripts (question, expected answer, generated answer, exact/F1 per
  item) were recorded for both; aggregate numbers are in the Results section below.
...
|---|---|---|---|---|---|
| SinLlama 1B (CPT + QA fine-tuned) | 22.88% | 0.505 | 12.59% | 100% | 0.00% |
| SinLlama 3B (CPT + QA fine-tuned) | 32.03% | 0.650 | 22.96% | 100% | 0.00% |

These numbers were produced using an evaluation procedure that exactly reproduces the
training-time inference path (same instruction text, context/question labels,
evidence-window retrieval, and grounding gate used during fine-tuning — verified to
construct byte-for-byte identical prompts). An earlier evaluation pass used a different
prompt format the models were never trained on and scored far lower for reasons
unrelated to model quality; the numbers above are within run-to-run noise of the
original training-time evaluation (22.22%/32.68% EM), confirming the corrected
procedure reproduces the intended result rather than either under- or over-stating it.
...
- Llama QA fine-tuning: complete for both 1B and 3B checkpoints, evaluated on the
  153-row external test split.
